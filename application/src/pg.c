...
int main(int argc, char *argv[])
{
    ...
    uint8_t shift = 0;          /// left shift input signal by these many bits
    ...
    case BLOCK_SIZE_1BYTE:
        ...
        for (c = 0; c<rcnt; c++) {
            ...
            replay_packet_8ch.clocks_to_next = ucnt;
            period_cnt += ucnt;
            c += ucnt - 1;
            ...
            printf("rp %d %d\n", replay_packet_8ch.sig, replay_packet_8ch.clocks_to_next);
        }

        replay_header.version = 1;
        replay_header.packet_count = pcnt; // to be replaced later
        ...
        free(sig_8ch);
        printf("%d data packets exported for a total of %d samples, %.02f ms\n", pcnt, period_cnt, (double) (period_cnt * sampling_interval) / 1000.0);
