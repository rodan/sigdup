...
void sys_messagebus_unregister_callback(void (*callback) (const uint16_t sys_message))
{
...
void sys_messagebus_unregister_msg(const uint16_t sys_message)
{
    struct sys_messagebus *p = messagebus, *pp = NULL;

    while (p) {
        if (p->listens == sys_message) {
            if (!pp) {
                messagebus = p->next;
            } else {
                pp->next = p->next;
