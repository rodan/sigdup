...
void sys_messagebus_register(
        ...
        const uint32_t listens
    );

/*!
	\brief Unregisters a node from the message bus based on the callback
	\sa sys_messagebus_register
*/
void sys_messagebus_unregister_callback(
        // the same callback used on sys_messagebus_register()
        void (*callback) (const uint16_t sys_message)
    );

/*!
	\brief Unregisters a node from the message bus based on the message
	\sa sys_messagebus_register
*/
void sys_messagebus_unregister_msg(
        // the same callback used on sys_messagebus_register()
        const uint16_t sys_message
    );
